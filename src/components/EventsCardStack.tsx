...
// above WorldBossCard, with no own-card treatment). Priority mirrors the
...
// is doing anything right now, World Boss keeps its original lead position.
...
export default function EventsCardStack({ characterId }: { characterId?: string | null }) {
  ...
  const worldBossColor: EventEmberColor | null = bossActive ? 'boss' : null
  ...
  const goldDonationLeads = !worldBossColor && !!goldDonationColor

  const worldBoss = characterId ? <WorldBossCard characterId={characterId} emberColor={worldBossColor} /> : null
  const goldDonation = <GoldDonationCard characterId={characterId} now={now} emberColor={goldDonationColor} />

  return (
    <>
      {goldDonationLeads ? goldDonation : worldBoss}
      {goldDonationLeads ? worldBoss : goldDonation}
    </>
  )
}
