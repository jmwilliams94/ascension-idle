...
export default function InventorySlot({
  ...
  if (!filled) {
    return (
      <div
        data-slot-id={slotId}
        style={{ borderColor: 'color-mix(in srgb, var(--ascension-gold) 40%, transparent)' }}
        className={`flex items-center justify-center rounded-lg border border-dotted bg-neutral-950/40 ${sizingClassName}`}
      >
  ...
  const button = (
    ...
    >
      ...
      {Boolean(compositionLevel) && (
        <span
          className="absolute right-1 top-0.5 z-10 text-[14px] font-extrabold leading-none text-slate-200"
