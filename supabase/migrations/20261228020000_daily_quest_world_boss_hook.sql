-- Hooks the World Boss Attacks daily quest into apply_world_boss_attack.
-- world_boss_participants.free_attempts_used/paid_attempts_used are keyed by
-- spawn_id+character_id and reset per-spawn, NOT daily (see CLAUDE.md), so
-- the quest needs its own counter via bump_daily_quest_progress rather than
-- reusing that column. Full body copied verbatim from
-- 20261212000000_zone_boss_reward_pool_restore_and_retune.sql, with one line
-- added right after the attempt actually consumes a free/paid slot (post
-- payment/cooldown/cap gates, so a refused attempt never counts). Same
-- signature, same grant (service_role only, not SECURITY DEFINER -- called
-- by the world-boss-attack edge function's service-role client).
begin;
...
begin
  ...
  -- Daily Quest hook: fires only once the attempt/cooldown/payment gates
  -- above have all passed, i.e. only on an attack that actually consumed a
  -- free/paid attempt -- a refused call never bumps progress.
  perform public.bump_daily_quest_progress(p_character_id, 'world_boss_attacks');
