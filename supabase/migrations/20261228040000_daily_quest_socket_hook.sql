-- Hooks the Socket Obtain daily quest into all 5 independent socket-granting
-- call sites (no shared helper exists between them -- confirmed by grep,
-- each function inlines its own roll). Full bodies copied verbatim from
-- their latest source migrations (quality_upgrade/level_upgrade/
-- master_forge_upgrade from 20261227000000_vip_forge_bank_material_client_
-- sync_fix.sql; level_upgrade_scroll from 20260922000000_restore_weapon_120_
-- master_forge_only.sql; quality_upgrade_scroll from
-- 20260930070000_block_pickaxe_quality_upgrade.sql), each with one line
-- added immediately after its own `v_socket_gained := true;`. All 5 are
-- SECURITY DEFINER, so the added call runs as the function owner regardless
-- of who invokes the RPC -- no grant changes needed.
begin;
...
begin
  ...
  if v_upgraded
    ...
    perform public.bump_daily_quest_progress(v_character_id, 'socket_obtain');
