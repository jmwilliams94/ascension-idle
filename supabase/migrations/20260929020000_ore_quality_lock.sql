-- Extends the just-restored promotion-material quality lock
-- (20260929010000_restore_promotion_material_quality_lock.sql) to Ore too
-- (slot_type = 'material', item_family = 'ore', the 30 Iron/Silver/Gold Ore
-- rows from 20260926010000_add_mining_ore_catalog.sql) — "none of the ores
-- can have a quality" (requested by the user). Without this, a player could
-- feed e.g. "Iron Ore (Rank 5)" into Quality Upgrade and get "Tempered Iron
-- Ore (Rank 5)": a nonsensical item that would also sell for more gold than
-- intended via sell_item's quality multiplier — the same class of problem
-- the original promotion-material lock existed to prevent, just not covered
-- by it since Ore uses a different slot_type. Confirmed via query that
-- slot_type = 'material' is used by nothing except these 30 Ore rows, so
-- broadening the guard to it has no other side effects.
--
-- Same three functions as the previous migration, condition widened from
-- `v_slot_type = 'promotion-material'` to
-- `v_slot_type in ('promotion-material', 'material')` — everything else
-- copied verbatim from the version just deployed a moment ago. Same
-- signatures throughout, so plain create-or-replace is safe.
begin;
...
begin
  ...
  if v_slot_type in ('promotion-material', 'material') then
...
begin
  ...
  if upgrade_type = 'quality' and v_slot_type in ('promotion-material', 'material') then
    return jsonb_build_object('ok', false, 'error', 'no_quality_upgrade_path');
  end if;
