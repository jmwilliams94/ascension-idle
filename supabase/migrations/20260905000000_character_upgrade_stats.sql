-- Per-character upgrade statistics (first entry in a planned Statistics
-- system, requested by the user): tracks lifetime successful Quality
-- Upgrade / Level Upgrade counts and a "dry streak" of successful upgrades
-- since the last armor-socket proc, tracked separately per upgrade type
-- since their socket-roll odds differ (1/100 quality vs 1/400 level, see
-- 20260817000000_socket_roll_rate_by_upgrade_type.sql). Streak is
-- per-character across all items, not per-item.
--
-- When a socket procs, a second global_announcements row now follows the
-- existing "<name>'s <item> gained its 1st/2nd socket!" one:
-- "<name>'s Quality Upgrade dry streak of <N> has ended." (or "Level
-- Upgrade"). N is the dry-streak length including the roll that just
-- procced, i.e. "it took N successful upgrades to get this socket."
--
-- record_successful_upgrade() is a small shared helper, modeled directly on
-- gold_donation_socket_multiplier() (20260830000000_forge_socket_gold_donation_buff.sql):
-- a plain (non-SECURITY DEFINER) function called from inside 5 already-
-- SECURITY DEFINER functions, which execute fine calling it since nested
-- calls resolve under the definer's own privileges -- no extra grant needed
-- for the internal call path, granted to authenticated anyway for
-- defense-in-depth/possible direct use later.
--
-- Same-signature create-or-replace on all 5 socket-rolling functions --
-- quality_upgrade, level_upgrade, master_forge_upgrade, level_upgrade_scroll,
-- quality_upgrade_scroll -- no drop needed. Bodies are otherwise unchanged
-- copies of their current definitions (20260830000000_forge_socket_gold_donation_buff.sql)
-- -- only the new streak-recording block is added, immediately after each
-- function's existing socket-roll block. Keep this shared block in sync
-- across all 5 if it changes again, same precedent as the socket-roll block
-- itself (see 20260814030000_socket_announcement_ordinal.sql's own note).
begin;

-- ============================================================================
-- 0. character_stats table
-- ============================================================================
create table if not exists public.character_stats (
  character_id uuid primary key references public.characters(id) on delete cascade,
  successful_quality_upgrades bigint not null default 0,
  quality_socket_dry_streak integer not null default 0,
  successful_level_upgrades bigint not null default 0,
  level_socket_dry_streak integer not null default 0
);

alter table public.character_stats enable row level security;

drop policy if exists character_stats_select_own on public.character_stats;
create policy character_stats_select_own
  on public.character_stats for select
  using (exists (select 1 from public.characters c where c.id = character_stats.character_id and c.account_id = auth.uid()));

grant select on public.character_stats to authenticated;
grant all on public.character_stats to service_role;

-- ============================================================================
-- 0b. record_successful_upgrade helper
-- ============================================================================
create or replace function public.record_successful_upgrade(
  p_character_id uuid,
  p_upgrade_type text,
...
begin
  if p_upgrade_type = 'quality' then
    ...
    values (p_character_id, 1, 1)
    on conflict (character_id) do update
      set successful_quality_upgrades = character_stats.successful_quality_upgrades + 1,
    ...
    values (p_character_id, 1, 1)
    on conflict (character_id) do update
      set successful_level_upgrades = character_stats.successful_level_upgrades + 1,
...
revoke all on function public.record_successful_upgrade(uuid, text, boolean) from public;
grant execute on function public.record_successful_upgrade(uuid, text, boolean) to authenticated;
...
begin
  ...
  if v_upgraded then
    v_dry_streak := public.record_successful_upgrade(v_character_id, 'level', v_socket_gained);
    if v_socket_gained then
      insert into public.global_announcements (kind, character_name, message)
      values (
        'socket_dry_streak_end',
...
-- ============================================================================
-- 4. level_upgrade_scroll -- streak recorded once per successful roll in the batch
-- ============================================================================
...
begin
  ...
  for i in 1..10 loop
    ...
    if v_upgraded then
      ...
      v_dry_streak := public.record_successful_upgrade(v_character_id, 'level', v_iter_socket_gained);
...
-- ============================================================================
-- 5. quality_upgrade_scroll -- streak recorded once per successful roll in the batch
-- ============================================================================
