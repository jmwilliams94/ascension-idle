-- Zone Boss exclusivity refinement (requested by the user) — the previous
-- rule (20261202000000_zone_boss_account_exclusivity.sql) blocked a
-- character outright if ANY other character on the same account had a real
-- attempt logged against the spawn. That's fine for a fresh spawn where only
-- one character has ever attacked it, but it deadlocked any spawn that
-- already had multiple contributing characters from the same account before
-- the previous migration deployed: character A sees character B has
-- attempts (blocked), character B sees character A has attempts (blocked) —
-- nobody on that account could attack the current boss at all anymore.
--
-- Fix: instead of "any other character blocks you," resolve the account's
-- single leading character for this spawn (highest total_damage among the
-- account's own participants, ties broken by earliest attempt) and only
-- allow that character to keep attacking. This self-heals any existing
-- spawn with multiple same-account contributors by picking a winner instead
-- of locking everyone out, and is equivalent to the old first-attacker rule
-- for a fresh spawn (only one character will ever accumulate damage once
