-- Retunes Level Upgrade's per-quality-tier success chance (requested by the
-- user, 2026-09-02): the old formula derived Tempered/Infused/Radiant/
-- Ascended from Normal's 100/80 range by halving each tier (x0.5 compounding
-- -> 50/40, 25/20, 12.5/10, 6.25/5). That's replaced with hand-tuned ranges
-- per tier instead of a single compounding multiplier:
--   Normal:   100 -> 80  (unchanged)
--   Tempered:  75 -> 50
--   Infused:   50 -> 25
--   Radiant:   25 -> 15
--   Ascended:  20 -> 10
-- Quality Upgrade's own 85->75 range and x0.58-per-tier multiplier are
-- unchanged. Master Forge's cost (1.5x expected manual cost, rounded up)
-- reads this same function, so it adjusts automatically -- no separate cost
-- table to touch. Same signature as the existing
-- compute_upgrade_success_chance_pct -- no drop needed, plain create or
-- replace.
begin;
