-- Continuation of 20260901060000_room_check_excludes_listed_and_mailed.sql --
-- draw_lucky_ticket/draw_lucky_ticket_bulk kept in their own file since their
-- full bodies are large. Same fix: v_gear_count now also excludes an
-- actively-listed Marketplace item and an item sitting in unclaimed Mail,
-- matching the client's useInventoryStore.occupiedSlotCount formula. Full
-- bodies copied verbatim from 20260901030000_moonbox_lucky_reward.sql
-- (current latest definition of both) except for that one query each.
begin;
