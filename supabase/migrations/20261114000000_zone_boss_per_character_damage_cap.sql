-- Zone Boss per-character damage cap (2026-11-14, requested by the user) —
-- fixes a real problem the rotation introduced: a max-level character's
-- single hit could two-shot a low-tier boss (e.g. Mourncrow, 3,640 HP)
-- before any other player had a chance to even see the spawn, defeating the
-- point of a shared, timed, multi-player event for the low-level zones.
--
-- Fix: cap each character's total contribution to a single spawn at 34% of
-- that boss's own max_hp — chosen deliberately (not a placeholder) so that
-- killing ANY boss, regardless of who's attacking it, requires damage from
-- at least 3 distinct characters (3 * 34% = 102% > 100%), and whoever hits
-- their own cap first is guaranteed the highest individual total_damage on
-- the spawn (locking in 1st place) since nobody else can exceed the same
-- cap. This is a per-character ceiling on cumulative damage across the
-- whole spawn, not a per-hit cap — a character can still land many small
-- hits or one huge hit, but never contribute more than 34% of max_hp total.
--
-- Critically, the credited/tallied damage (v_effective_damage) is now also
-- what reduces the boss's current_hp — previously current_hp was reduced by
-- the raw p_damage while only the *tally* was clamped for overkill. If HP
-- reduction still used raw p_damage, a single overpowered hit would still
-- nearly delete the boss even though that character's own tally was capped,
