-- Lucky Lad's free 4h ticket cooldown lived on characters.lucky_free_ticket_claimed_at
-- -- per-character, not per-account. Since character deletion cascades (see
-- CLAUDE.accounts-and-classes.md's Character deletion note), a player could
-- claim the free ticket, delete the character, create a new one in the same
-- slot, and claim again immediately -- an unlimited free-ticket farm
-- (reported by the user). Moves the cooldown to players.lucky_free_ticket_claimed_at
-- (account-wide, matching ascension_points/unlocked_classes' own trust model)
-- so it survives character deletion.
begin;

alter table public.players
  add column if not exists lucky_free_ticket_claimed_at timestamptz;

-- Preserve whichever existing claim is most recent across a player's characters
-- so this migration doesn't hand every existing account a free extra draw.
update public.players p
set lucky_free_ticket_claimed_at = c.max_claimed_at
from (
  select account_id, max(lucky_free_ticket_claimed_at) as max_claimed_at
  from public.characters
  group by account_id
) c
where c.account_id = p.id and c.max_claimed_at is not null;
...
begin
  ...
  -- Account-wide cooldown + AP balance, locked together so a racing call
  -- can't read a stale ascension_points value out from under the update below.
...
alter table public.characters
  drop column if exists lucky_free_ticket_claimed_at;
