-- Revert: 20261125000000_pvp_duel_disable_timer_for_testing.sql disabled the
-- 15s PvP duel turn timer entirely (every turn_deadline write replaced with
-- null) and was explicitly marked "do not leave this in place for real
-- matchmaking (Phase 3)" -- it was never reverted before Phase 3 (the weekly
-- tournament, 20261127000000_pvp_tournament_core.sql) went live, so today's
-- (2026-09-04) Friday-noon-Brisbane kickoff produced a real bracket match
-- with no forfeit-on-timeout possible. Restores the exact pre-testing values
-- from 20261124000000_pvp_duel_symmetric_hiding.sql: now() + interval '15
-- seconds' on duel start and every place_zone/non-final guess, null only
-- when a guess ends the duel (v_duel_over). Same signatures as before (only
-- bodies change, per 20261128000000's own note), so no `drop function`
-- needed. pvp_duel_gather_state is untouched -- it only reads turn_deadline,
-- never writes it.
begin;
