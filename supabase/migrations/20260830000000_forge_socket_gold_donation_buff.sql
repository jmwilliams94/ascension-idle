-- Wires the Gold Donation Event's 'socket_unlock' buff into every Forge RPC
-- that rolls an armor socket chance — quality_upgrade, level_upgrade,
-- master_forge_upgrade, level_upgrade_scroll, quality_upgrade_scroll. None
-- of these live in resolve-combat/combatResolver.ts (socket rolls are a
-- Forge mechanic, not a combat one), so this is a separate migration from
-- the resolve-combat wiring in 20260829000000.
--
-- One reusable helper (gold_donation_socket_multiplier) avoids duplicating
-- "is there an active socket-category event" 5 times. It's a plain `stable`
-- function, not SECURITY DEFINER — called from inside 5 already-SECURITY-
-- DEFINER functions, which execute with their owner's privileges regardless
-- (see world_boss_reward_for_tier's own comment on this same point), so no
-- extra grant is needed for the internal call path; granted to authenticated
-- anyway for defense-in-depth/possible direct use later.
--
-- Same-signature create-or-replace on all 5 functions, no drop needed.
-- Bodies are otherwise unchanged copies of each function's current
-- definition (quality_upgrade/level_upgrade: 20260816000000 and
-- 20260817000000; master_forge_upgrade/level_upgrade_scroll: 20260817000000;
-- quality_upgrade_scroll: 20260814030000) — only the socket-roll-chance
-- usage changes. The two scroll (batch-of-10) functions compute the
-- multiplied chance once before their loop rather than re-querying per
-- iteration, since the active event can't change mid-call.
begin;

create or replace function public.gold_donation_socket_multiplier()
returns numeric
language sql
stable
as $$
  select coalesce(
    (select p.buff_multiplier
     from public.gold_donation_state s
     join public.gold_donation_pools p on p.id = s.current_pool_id
     where s.id = 1 and p.status = 'active' and p.buff_category = 'socket_unlock' and now() < p.buff_ends_at),
    1
  );
$$;

revoke all on function public.gold_donation_socket_multiplier() from public;
grant execute on function public.gold_donation_socket_multiplier() to authenticated;
...
-- ============================================================================
-- 4. level_upgrade_scroll -- multiplier computed once before the 10x loop
-- ============================================================================
...
-- ============================================================================
-- 5. quality_upgrade_scroll -- multiplier computed once before the 10x loop
-- ============================================================================
