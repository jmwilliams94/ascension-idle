-- Blocks the standard Forge Quality Upgrade path for Pickaxe (requested by
-- the user), now that Tier Up (pickaxe_tier_upgrade) directly sets the same
-- item_instances.quality_tier field -- without this, a player could bypass
-- Tier Up's guaranteed-success gold+gem cost entirely by rolling cheap,
-- probabilistic Fallen Star Quality Upgrades instead, since both paths
-- mutate the exact same column. Guard added to quality_upgrade,
-- quality_upgrade_scroll, and master_forge_upgrade's quality branch, same
-- shape as the existing promotion-material/material slot_type lock. Bodies
-- are otherwise an unchanged copy of the latest versions
-- (20260929020000_ore_quality_lock.sql) -- diffed against that file
-- directly, not a stale copy, per this project's own recurring lesson about
