-- Lucky Lad: bulk draw — "Open All 9" for 160 AP (8x the single-card 20-AP
-- cost — "pay for 9 for the cost of 8", requested by the user), a third
-- payment path alongside the free-ticket/1-ticket and single-card 20-AP
-- draws in draw_lucky_ticket. Unlike that single draw, every one of the 9
-- rolled cards grants a real, independent reward here — all 9 are resolved
-- and granted server-side in one call, the moment payment clears. The client
-- (LuckyPanel.tsx) then reveals the already-decided board one card at a time
-- purely as a client-side animation gate — nothing is left undecided once
-- payment clears, so there's no new cheating surface (same reasoning
-- draw_lucky_ticket's own header gives for the single-draw flow).
...
begin
  ...
  for i in 0..8 loop
    ...
    if v_kind = 'comet' then
      ...
      v_comet_count := v_comet_count + v_amount;
    ...
    if v_kind = 'comet_box' then
      ...
      values ('lucky_comet_box', v_character_name, v_character_name || ' won a Comet Box (+100 Comets) from LL!');
    elsif v_kind = 'comet_scroll' then
      insert into public.global_announcements (kind, character_name, message)
