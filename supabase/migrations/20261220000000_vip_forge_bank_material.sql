-- VIP: let Forge draw Comets/Fallen Stars straight from the account Bank
-- when a character comes up short (requested by the user). New helper
-- `ensure_forge_currency` wraps the existing `ensure_loose_currency`
-- (untouched -- still used as-is by promote_character, which deliberately
-- does NOT get bank fallback; this is scoped to Forge only) and, only if a
-- shortfall remains after scroll-unbundling, draws it from
-- players.bank_comets/bank_fallen_stars -- the points-style account Bank
-- balance shown in BankSquares.tsx, NOT comet_bank_count/fallen_star_bank_count
-- (that physical-tile path's deposit side is UI-retired and can no longer be
-- topped up, so it would be a dead end for an auto-use feature). Gated on
-- real VIP (vip_expires_at) + a new per-currency toggle,
-- characters.vip_automation_settings -> 'autoUseBankMaterial' -> 'comet'/
-- 'fallen_star'. Wired into quality_upgrade/level_upgrade/master_forge_upgrade
-- -- the two Scroll-batch functions (quality_upgrade_scroll/
-- level_upgrade_scroll) spend a whole Scroll unit directly and never call
-- ensure_loose_currency at all, so they're out of scope here.
--
-- Also fixes a real pre-existing bug found while touching this function:
-- set_vip_automation_settings' keyword whitelist was never updated for
-- `autoUsePotions` (shipped v1.127.0, see
-- 20261209020000_mp_potion_auto_use_offline.sql, which reads it back) -- every
-- attempt to toggle Auto-Use HP/MP Potions has been silently dropped
-- server-side since it shipped (confirmed live against Switchee's real
-- vip_automation_settings row, which has every other toggle but no
-- autoUsePotions key at all). Restored here alongside the new
-- autoUseBankMaterial key, same migration since both touch the same
-- whitelist object.
begin;

-- ============================================================================
-- 1. ensure_forge_currency -- new function, Forge-only currency-shortfall
--    resolver. Same positional signature as ensure_loose_currency so the 3
--    Forge call sites below are a one-line swap each.
-- ============================================================================
...
begin
  ...
  -- Not VIP, or VIP but this currency's auto-use toggle is off -- fall
  -- through unchanged; the caller's own affordability check reports the
  -- ordinary "not enough" error.
  if v_account_id is null or v_vip_expires_at is null or v_vip_expires_at <= now()
     or coalesce((v_automation -> 'autoUseBankMaterial' ->> p_currency_type)::boolean, false) is not true then
    return v_result;
  end if;
...
revoke all on function public.ensure_forge_currency(uuid, text, integer) from public;

-- ============================================================================
-- 2. quality_upgrade -- unchanged body (latest source:
--    20260930070000_block_pickaxe_quality_upgrade.sql) except its
--    ensure_loose_currency call becomes ensure_forge_currency.
-- ============================================================================
...
-- ============================================================================
-- 3. level_upgrade -- unchanged body (latest source:
--    20260922000000_restore_weapon_120_master_forge_only.sql) except its
--    ensure_loose_currency call becomes ensure_forge_currency.
-- ============================================================================
...
-- ============================================================================
-- 4. master_forge_upgrade -- unchanged body (latest source:
--    20260930070000_block_pickaxe_quality_upgrade.sql) except its
--    ensure_loose_currency call becomes ensure_forge_currency.
-- ============================================================================
...
-- ============================================================================
-- 5. set_vip_automation_settings -- whitelist gains autoUseBankMaterial
--    (new) and autoUsePotions (real pre-existing gap, fixed here -- see this
--    migration's header). Same signature, plain create-or-replace.
-- ============================================================================
...
begin
  ...
  v_clean := jsonb_build_object(
    ...
    'autoUseBankMaterial', jsonb_build_object(
      'comet', coalesce((p_settings->'autoUseBankMaterial'->>'comet')::boolean, false),
      'fallen_star', coalesce((p_settings->'autoUseBankMaterial'->>'fallen_star')::boolean, false)
    )
  );
