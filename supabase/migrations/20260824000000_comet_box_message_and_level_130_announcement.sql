-- Two independent announcement tweaks, requested by the user together:
--
-- 1. Drop "(+100 Comets)" from the Comet Box Lucky Lad announcement message
--    -- the amount is redundant now that the reveal card/board tile already
--    shows it, and the client now shows a dedicated Comet Box icon instead
--    of reusing the plain Comet one (see COMET_BOX_ICON_SRC in
--    forgeCosts.ts), so the parenthetical felt doubly unnecessary. Full-body
--    copies of draw_lucky_ticket (unchanged since 20260821080000) and
--    draw_lucky_ticket_bulk (unchanged since 20260822000000), each with only
--    that one announcement message string edited -- same "no signature
--    change, no drop needed" situation as every other in-place SQL text
--    tweak in this project.
--
-- 2. New "reached level 130" global announcement -- first ever
--    combat-triggered announcement (every other kind so far comes from
--    Lucky Lad draws or Forge socket procs). resolve_combat_apply_results
--    already receives the character's new level as p_level on every single
--    resolve-combat call (leveled or not), but not the *old* level, so it
--    can't tell a crossing from a no-op without looking it up -- rather than
--    widen this function's already-long positional argument list (which
--    would hit the "argument-list change needs an explicit drop first" gotcha,
--    see CLAUDE.md), it just does one extra by-primary-key SELECT for the
--    character's pre-update level (cheap, no separate round trip from the
--    Edge Function's perspective, and this function already runs serialized
--    per-character behind resolve_combat_gather_state's optimistic-CAS
--    claim). Fires once, exactly on the tick that actually crosses the
--    threshold (v_old_level < 130 and p_level >= 130), never again after.
--    130 is a flat, hardcoded threshold -- not a general milestone system.
begin;
...
begin
  ...
  if v_kind = 'comet' then
    ...
    v_new_comet_count := v_comet_count + v_amount;
...
-- resolve_combat_apply_results: unchanged from 20260821060000_consolidate_resolve_combat.sql
-- except v_old_level/v_character_name added and the level-130-crossing
-- announcement insert right after the level UPDATE.
...
begin
  ...
  select level, name into v_old_level, v_character_name
  from public.characters where id = p_character_id;
  ...
  if v_old_level < 130 and p_level >= 130 then
    insert into public.global_announcements (kind, character_name, message)
    values ('level_130', v_character_name, v_character_name || ' reached level 130!');
  end if;
