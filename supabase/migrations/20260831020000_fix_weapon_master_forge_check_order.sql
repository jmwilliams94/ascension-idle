-- Fixes an ordering bug in 20260831010000: level_upgrade/level_upgrade_scroll
-- checked "weapon at 120+ needs Master Forge" BEFORE checking whether the
-- item was already at the top of its chain (level 130, no next template) --
-- so an already-maxed weapon reported "use Master Forge" instead of
-- "already at max level" from the regular Forge, even though Master Forge
-- would immediately reject it with already_max_level too (that check runs
-- after the currency branch there, unaffected by this bug). Swaps the two
-- checks so already_max_level always wins when both would apply.
--
-- Same-signature create-or-replace, no drop needed.
begin;
...
begin
  ...
  -- even start here (checked after already_max_level, same reasoning as
  -- level_upgrade above).
  if v_slot_type = 'weapon' and v_required_level >= 120 then
    return jsonb_build_object('ok', false, 'error', 'weapon_requires_master_forge', 'level', v_current_level);
  end if;
