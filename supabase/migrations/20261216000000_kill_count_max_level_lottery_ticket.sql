-- Bounty Hunt (kill_count) daily quest paid its EXP reward unconditionally,
-- unlike quality_order which already branches on max level (see
-- 20260906072203_daily_quest_reward_ball_count.sql). At level 130 the EXP
-- gain was silently discarded: claim_daily_quest's level-up loop never runs
-- once v_new_level is already 130, and the `if v_new_level >= 130 then
-- v_new_exp := 0` line then zeroes out the exp field the reward had just
-- added to -- a level-130 character got a Comet Scroll and nothing else.
-- Mirrors quality_order's fix: max-level characters get 1 Lottery Ticket
-- (equivalent to quality_order's 1-ball value) instead of the EXP, keeping
-- the existing 1 Comet Scroll unchanged. Full body otherwise unchanged from
-- the live version.
begin;
...
begin
  ...
  if v_type = 'quality_order' then
    ...
    if v_level >= 130 then
      ...
      set lottery_ticket_count = lottery_ticket_count + 1, comet_scroll_count = comet_scroll_count + 1
      where id = p_character_id
      returning lottery_ticket_count, comet_scroll_count into v_lottery_tickets, v_comet_scroll_count;

      v_reward := jsonb_build_object(
        'kind', 'lottery_ticket_and_comet_scroll', 'amount', 1, 'lottery_ticket_count', v_lottery_tickets,
