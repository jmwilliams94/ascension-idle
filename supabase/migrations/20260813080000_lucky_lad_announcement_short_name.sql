-- Shorten "Lucky Lad" to "LL" in global announcement text (2026-08-13,
-- requested by the user) -- scoped to the announcement messages only, not
-- the Lucky Lad tab/mascot naming elsewhere in the app. Unchanged 3-arg
-- signature, so plain create-or-replace is safe (see the recurring
-- "dropping a signature first" gotcha noted elsewhere in this project for
-- when that ISN'T true). Body is otherwise identical to
-- 20260813010000_split_lucky_gem_rewards_by_type.sql's draw_lucky_ticket --
-- only the 8 announcement message strings' trailing "from Lucky Lad!" changed
-- to "from LL!". GlobalAnnouncementTicker.tsx's resolveAnnouncementIconSrc
-- regex for lucky_gear_ascended_random was updated to match in the same pass.
