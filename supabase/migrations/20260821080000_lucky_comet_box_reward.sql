-- Lucky Lad: new "Comet Box" reward kind — a flat, instant +100 Comets,
-- distinct from the existing plain 'comet' kind (which grants +1 loose
-- Comet, same as a kill-drop). Requested by the user.
--
-- Weight: 1.0, PLACEHOLDER (an intentional, adjustable balance number, not a
-- final decision — flag for revisit like every other odds/economy constant
-- in this project). Chosen to sit noticeably rarer than Comet Scroll's own
-- weight (8.0, worth 10 loose Comets) since +100 Comets is worth roughly 10x
-- a Comet Scroll — 1.0 lands it about 8x rarer than a Scroll, in the
-- 0.5-2.0 "noticeably rarer" range asked for.
--
-- pick_lucky_reward's weights are meant to sum to exactly 100 (see this
-- function's own v3 header) so every reward stays reachable by v_roll's
-- 0-100 range — adding a new 1.0 weight without offsetting it would silently
-- shave 1.0 of probability mass off the tail (the Hyper Rare gear kinds),
-- since v_roll never reaches 100. Offset by trimming Class 1 Money Bag
-- (the single largest weight, 16.3 -> 15.3) rather than touching anything
-- in the already-thin Rare/Hyper Rare bands. New totals: Common 60.9 /
-- Uncommon 28.0 / Rare 10.9 (was 9.9, +1.0 for Comet Box) / Hyper Rare 0.2.
...
begin
  for v_row in
    select * from (values
      -- ===== Common (60.9, was 61.9 -- Class 1 Money Bag trimmed 1.0 to
      -- fund Comet Box below) =====
      ('money_bag', 1, 15.3::numeric),
      ...
      -- ===== Rare (10.9, was 9.9 -- +1.0 for Comet Box) =====
      ('fallen_star', 1, 4.0::numeric),
      ('gem_ascended', 1, 3.0::numeric),
      ('comet_box', 100, 1.0::numeric),
...
-- draw_lucky_ticket: add the comet_box grant branch (flat +N Comets, N
-- carried in the reward's own `amount` field so the grant amount can be
-- retuned later without another migration) plus its own announcement, at the
-- same "at least as rare as Comet Scroll" threshold the rest of the
-- Announcements scope already uses (Comet Box's weight of 1.0 is rarer than
-- Comet Scroll's 8.0, so it qualifies). No room check needed — like 'comet'/
-- 'fallen_star', this is a currency grant, not a physical item. Unchanged
-- 3-arg signature (create or replace is safe); body otherwise identical to
-- 20260821070000_lucky_free_ticket_cooldown_4h.sql's version (4-hour
-- cooldown carried forward unchanged).
