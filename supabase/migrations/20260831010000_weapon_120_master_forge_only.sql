-- Revises 20260831000000's approach per the user: weapons past level 120
-- can no longer be Level Upgraded at the regular Forge at all (single
-- attempt OR Comet Scroll batch) -- that path is now Master-Forge-exclusive,
-- flat 1 Fallen Star per level (no RNG cost formula, since there's no more
-- "manual" cost to price Master Forge at 1.5x of, past this point). Below
-- 120, nothing changes: both Forge and Master Forge still work exactly as
-- before, priced in Comets.
--
-- 1. level_upgrade -- reverts the fallen_star currency-switch added in
--    20260831000000 (no longer needed, since weapon+required_level>=120 now
--    refuses outright instead of charging a different currency) and adds
--    the new upfront refusal.
-- 2. master_forge_upgrade -- keeps the fallen_star currency-switch from
--    20260831000000, but overrides the cost to a flat 1 (instead of the
--    usual ceil(1/success*1.5) formula) whenever that switch is active.
-- 3. level_upgrade_scroll -- refuses upfront on a weapon already at 120+
--    (same as level_upgrade), and now also stops mid-batch the moment a
--    weapon would cross above 120, wasting any remaining rolls with no
--    refund (same "wasted, no refund" precedent as topping out or hitting
--    the equipped-item level ceiling).
--
-- Same-signature create-or-replace on all three, no drop needed.
begin;
...
begin
  ...
  -- Weapons at level 120+ are Master-Forge-exclusive from here -- see this
  -- migration's header.
...
begin
  ...
  if upgrade_type = 'quality' then
    ...
    -- Weapon-only, level-120+ Fallen Star cost -- Master Forge is now the
    -- *only* way to Level Upgrade a weapon past 120, at a flat 1 Fallen Star
    -- per level (no cost formula -- see this migration's header).
...
begin
  ...
  -- even start here (see this migration's header; matches level_upgrade's
  -- own upfront refusal).
  if v_slot_type = 'weapon' and v_required_level >= 120 then
    return jsonb_build_object('ok', false, 'error', 'weapon_requires_master_forge', 'level', v_current_level);
  end if;
