-- Restores the "promotion-material items (Lunar Chest/Jade Shard/Umbrite
-- Ore) can't be Quality Upgraded into nonsense like 'Tempered Umbrite Ore'"
-- guard, originally added in 20260906000000_lunar_chest_rename_and_quality_lock.sql
-- to quality_upgrade/master_forge_upgrade/quality_upgrade_scroll.
--
-- Root cause: exactly the regression class CLAUDE.gear-and-forge.md already
-- warns about — 20260918000000_dry_streak_counts_failures.sql redefined all
-- three functions to add dry-streak tracking, but (per that file's own
-- lesson about 20260905000000_character_upgrade_stats.sql making the same
-- mistake with weapon-120 logic) its "carried forward" bodies were based on
-- versions that predate the quality-lock migration, silently dropping the
-- guard. Confirmed live in production: all three functions' actual deployed
-- source has zero occurrences of 'promotion-material' as of this migration.
--
-- This was dormant/unreachable for Umbrite Ore specifically since it had no
-- acquisition path until Mining shipped (20260929000000_umbrite_ore_mining_drop.sql,
-- same day) — now that it's a real obtainable item, shipping it without this
-- guard would let a player Quality-Upgrade it into "Tempered Umbrite Ore",
-- the exact nonsense this system exists to prevent. Jade Shard/Lunar Chest
-- were already reachable and already exposed to this regression; this fixes
-- all three, not just the one this session happens to be touching.
--
-- Bodies below are the actual current-latest per function (verified via
-- direct pg_proc.prosrc query against the linked project, not just the
-- migration file grep, precisely to avoid the "diffed against a stale
-- version" mistake this migration is itself fixing) — only the one guard
-- clause is new in each, everything else copied verbatim. Same signatures
-- throughout, so plain create-or-replace is safe.
begin;
...
-- ============================================================================
-- 2. master_forge_upgrade (guard only applies to the quality branch, mirrors
--    the original 2026-09-06 placement)
-- ============================================================================
