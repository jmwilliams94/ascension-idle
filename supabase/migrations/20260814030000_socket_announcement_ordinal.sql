-- Armor-socket announcement now says which socket was gained (2026-08-14,
-- requested by the user) — "<name>'s <item> gained its 1st socket!" /
-- "... gained its 2nd socket!" instead of the ambiguous "gained a socket!"
-- for both. v_socket_count is already computed before the socket is
-- appended in every one of these functions (it's also the "does this item
-- already have 2 sockets" gate condition), so 0 = about to become the 1st,
-- 1 = about to become the 2nd — no new lookup needed, just a new
-- v_socket_ordinal local built off the existing count.
--
...
-- copies of their current definitions (20260808050000_global_announcements.sql,
-- 20260813140000_forge_equipped_level_guard.sql,
-- 20260810030000_fix_socket_announcement_stale_name.sql,
-- 20260813090000_forge_scroll_batch_upgrade.sql) — keep the shared socket-roll
-- block in sync across all 5 if it changes again.
--
-- Client-side: GlobalAnnouncementTicker.tsx's resolveAnnouncementIconSrc
-- parses this same message shape via regex to find the gear icon — updated
-- alongside this migration to match the new "gained its Nst/nd socket!"
-- wording.
begin;
