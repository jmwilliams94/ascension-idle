...
- **Implemented**: a single fixed 40-cell grid (`INVENTORY_SLOT_CAP` in `useInventoryStore.ts`), always rendering all 40 cells regardless of how many are occupied — empty cells render dimmed/unclickable (`InventorySlot`, a deliberately dumb/presentational per-cell component keyed by a stable `slotId`, plus optional `draggable`/`onDragStart`/`onDragEnd` props used only by Forge). `InventoryPanel` (the grid + detail-card component) lives **only inside the Forge overlay** now (`ForgePanel.tsx`) — it is no longer shown in the side HUD (`SideHud.tsx` shows just Stats). This is a deliberate move, not an oversight: Forge's drag-and-drop needs the grid rendered inline next to its Upgrade Slot, and showing it in two places at once (side HUD + Forge) would let the same item appear draggable/duplicated in both. `InventoryPanel` takes two optional props enabling this — `reservedItemId` (renders that cell empty, since the item is "in" Forge's Upgrade Slot) and `onItemDragStart` (opts gear tiles into `draggable`) — both no-ops when omitted, so any future non-Forge usage gets the exact same plain-grid behavior as before. The real scaling-by-level model (cap growing from 30 up to 40 via bag item upgrades, maxing around character level 67) is **not built yet** — every character is currently treated as already at the max 40 — unresolved per CLAUDE.md, exact intermediate tier breakpoints still need a follow-up pass if we replicate it precisely later.
