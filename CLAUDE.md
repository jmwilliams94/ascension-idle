...
- **Shop overlay pattern**: a 5th "Shop" button in `BottomNav` opens `ShopOverlay`, which renders as an absolutely-positioned overlay on top of `GameCanvas` (not in place of `BottomNav`, which stays visible/usable underneath) and is closable via an X (`useShopStore`) — distinct from the `HudTabs`/`useHudTabStore` tab-switching pattern (Zone/Equipment/Forge/Market), which has no "closed" state. Non-Hunter classes see "Nothing available yet" in the shop.
