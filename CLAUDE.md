...
- **Central character placeholder (`PaperDollBody.tsx`, confirmed abstract/geometric — no character art, matching the game's "greybox" visual identity)**: a segmented rectangle, not a silhouette. Three stacked horizontal bands represent Headgear (top), Body/Armor (middle), and Boots (bottom); Weapon isn't body-worn, so it's a thin colored accent along the left edge instead of a fourth band. Each band/accent's color reflects the `quality_tier` of whatever's equipped there via `getSlotVisual` in `equipmentBonus.ts` (same `QUALITY_COLORS` gradient used everywhere else — gray/blue/purple/orange/red — at reduced opacity over the band), or a neutral dark fill (`rgba(30,41,59,0.6)`) when nothing's equipped or the slot has no item system yet. Only **Super** quality gets a glow — a slow (~2.6s) pulsing `box-shadow` (`.super-quality-glow` in `index.css`, deliberately not applied at any other tier so it reads as genuinely special rather than decoration every item gets. Headgear/Body/Boots are hardcoded to pass `undefined` into `getSlotVisual` today (no item system exists for them yet, so they always render neutral) — the component takes each slot's `quality_tier` as a prop, so wiring in a real value once those slot types are built is enough to light them up automatically, no rebuild required. Main Hand is the only one wired to real data (`equippedItem?.quality_tier`) today.
- Hover tooltips beyond the native `title` attribute (used for the Weapon slot, the locked placeholders, and each paper-doll band/accent) are not built — the richer Diablo/PoE-style tooltip described above is still aspirational.
