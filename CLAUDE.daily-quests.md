...
- `src/components/DailyQuestBadge.tsx` — fixed floating button, bottom-right corner (`right-3`), positioned opposite `UnclaimedLootBadge.tsx` (bottom-left). Always mounted, hidden via CSS (`invisible pointer-events-none opacity-0`) rather than `return null` — a conditionally-rendered version pops a brand-new `position: fixed` element into the DOM right after every page load once quest data resolves, which was implicated in a MobileBottomNav scroll-drift regression (see the nav's own resume-nudge fix history). Deliberately has no `backdrop-blur`/forced `translateZ(0)` layer for the same reason. Its badge count is `isQuestClaimable` count (quests actually ready to claim), not just "not yet claimed" — a quest still in progress doesn't light up the badge.
